  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-8 px-4 py-8 lg:grid-cols-[1fr_300px]">
      <div className="min-w-0">
        ...
        <p className="mt-2 text-ink-600">{intro}</p>
        ...
        <section className="mt-10">
          <h2 className="mb-3 text-xl font-bold tracking-tight text-ink-900">Otras calculadoras</h2>
          <ul className="grid gap-2 sm:grid-cols-2">
            {related.map((c) => (
              <li key={c.slug}>
                <Link
                  href={`/calculadora-${c.slug}`}
                  className="block rounded-xl border border-ink-200 bg-white px-4 py-2.5 text-sm transition hover:border-brand-300 hover:text-brand-600 hover:shadow-card"
                >
                  {c.title}
                </Link>
              </li>
