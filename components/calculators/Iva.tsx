    ...
    <div className="surface-card p-5">
      <div className="mb-4 flex flex-wrap gap-2">
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium ${
            mode === "add" ? "bg-brand-600 text-white" : "bg-ink-100 text-ink-700"
          }`}
        >
          Añadir IVA
        </button>
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium ${
            mode === "remove" ? "bg-brand-600 text-white" : "bg-ink-100 text-ink-700"
          }`}
        >
          Quitar IVA
        </button>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <div className="result-card">
          <p className="text-sm text-ink-600">Base imponible</p>
          <p className="text-xl font-bold text-brand-700">{formatEUR(base)}</p>
        </div>
        <div className="result-card">
          <p className="text-sm text-ink-600">IVA ({rate}%)</p>
          <p className="text-xl font-bold text-brand-700">{formatEUR(vat)}</p>
        </div>
        <div className="result-card">
          <p className="text-sm text-ink-600">Total con IVA</p>
